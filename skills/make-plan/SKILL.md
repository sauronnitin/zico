---
name: make-plan
description: "Create a memory-aware implementation plan that builds on past session context. ONLY use when planning features that continue previous work, reference past decisions, or need session history to be accurate. Triggers: 'plan next phase', 'plan based on what we built', 'continue our plan', 'plan with context from last session'. Do NOT use for greenfield tasks — use writing-plans instead."
---
