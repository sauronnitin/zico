---
name: do
description: "Execute a plan that was created by make-plan, using memory context from past sessions. Use when carrying out a memory-aware plan that references previous work. Triggers: 'execute the plan', 'run the plan', 'do it' (after make-plan output). Do NOT use for saved plan files — use executing-plans. Do NOT use for current-session parallel tasks — use subagent-driven-development."
---
